...
# ==== НАСТРОЙКИ ====
SEARCH_URL = os.environ.get("SOMON_URL", "https://m.somon.tj/telefonyi-i-svyaz/mobilnyie-telefonyi/")
...
def log_market_point(item):
    """Дешёвая запись для базы цен — без доп. запросов к сайту, без Gemini."""
    with open(PRICE_HISTORY_FILE, "a", encoding="utf-8") as f:
        f.write(json.dumps({
            ...
            "memory_gb": item.get("memory"), "date": time.strftime("%Y-%m-%d"),
        }, ensure_ascii=False) + "\n")
...
def market_stats_for(title, exclude_id):
    """Медиана/минимум по похожим прошлым объявлениям из своей же базы."""
...
def fetch_listings():
    """Лёгкий проход по странице категории — без захода в каждое объявление."""
...
def analyze_listing(item, photo_urls, stats):
    ...
    for attempt in range(2):
        try:
            ...
            print(f"Сбой Gemini (попытка {attempt + 1}):", e)
        time.sleep(5)
...
def main():
    ...
    # Шаг 1: дёшево логируем ВСЕ объявления в базу цен (без сети, без Gemini)
    ...
    # Шаг 2: только до MAX_NEW_ITEMS_PER_RUN кандидатов — полный анализ
    candidates = [
        item for item in listings
        if selected(item, searches, mode) and not seen.get(item["id"], {}).get("photo_ok")
    ...
    for item in candidates:
        ...
        try:
            ...
            stats = market_stats_for(item["title"], item["id"])
